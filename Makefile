...
phpcbf:
#	conditional-directive that expands all variable references in arg1 and arg2 and compares them.
#	If they are different the text-if-true is effective otherwise the text-if-false, if any, is effective:
# ifneq ($(wildcard test),)
...
phpunit: prepare
	...
	# [ ! -d "test" ] || $(PHPUNIT) --configuration phpunit.xml.dist $(options)
	# [ ! -d "tests" ] || $(PHPUNIT) --configuration phpunit.xml.dist $(options)
	# [ ! -d "tests" ] || XDEBUG_MODE=coverage $(PHPUNIT) --configuration phpunit.xml.dist $(options)
